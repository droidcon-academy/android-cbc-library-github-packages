...
android {
    namespace = "dev.sunnat629.stringextensions"
    compileSdk = 34
...
afterEvaluate {
    publishing {
        ...
        val testImplementationDependencies = configurations["testImplementation"].allDependencies.toList()
        val androidTestImplementationDependencies = configurations["androidTestImplementation"].allDependencies.toList()

        val addDependenciesToPom: (List<Dependency>, Node) -> Unit = { dependencies, dependenciesNode ->
        ...
        publications {
            listOf("debug", "release").forEach { publicationType ->
                create<MavenPublication>(publicationType) {
                    logger.lifecycle("Get version. version = $version")

                    groupId = "droidcon-academy"
                    artifactId = "stringextensions${if (publicationType == "release") "" else "-$publicationType"}"
                    version = version
                    artifact("build/outputs/aar/${project.name}-$publicationType.aar")
